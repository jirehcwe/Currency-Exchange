...
public class InputHandler : MonoBehaviour {
    ...
    void Update()
    {
        ...
        int balance = W_MapKeys() - T_MapKeys() + Draggable.getDragSum();
    ...
    int T_MapKeys()
    {
        if (Input.inputString != "")
        {
            ...
            int value = Table.transform.GetChild(T_index).GetComponent<Draggable>().valueOfNote;
            if (T_index < Table.transform.childCount)
            {
                ...
                {
                    if (value > Wallet.transform.GetChild(i).GetComponent<Draggable>().valueOfNote)
                    {
                        newSiblingIndex = i;
                        break;
                    }
    ...
    int W_MapKeys()
    {
        if (Input.inputString != "")
        {
            ...
            int value = Wallet.transform.GetChild(W_index).GetComponent<Draggable>().valueOfNote;
            ...
            {   
                ...
                for (int i = 0; i < Table.transform.childCount; i++)
                {
                    ...
                    if (value > Table.transform.GetChild(i).GetComponent<Draggable>().valueOfNote)
                    {
                        newSiblingIndex = i;
                        break;
                    }
